...
void do_table() {
  outfile << "<h1>Path Decomposition Table</h1>\n"
             "<p>Shaded entries indicate cases where <i>POSIX</i> and "
             "<i>Windows</i>\n"
             "implementations yield different results. The top value is the\n"
             "<i>POSIX</i> result and the bottom value is the <i>Windows</i> "
             "result.\n"
  ...
  while (std::getline(infile, test_case)) {
    ...
    if (test_case.empty() || test_case[0] != '#') do_row(test_case);
  }
...
{
  if (argc != 5) {
    std::cerr << "Usage: path_table \"POSIX\"|\"Windows\" input-file "
                 "posix-file output-file\n"
