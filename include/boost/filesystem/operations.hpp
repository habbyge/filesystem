...
// These includes are left for backward compatibility and should be included
// directly by users, as needed
...
BOOST_SCOPED_ENUM_UT_DECLARE_BEGIN(copy_options, unsigned int){
    none = 0u,  // Default. For copy_file: error if the target file exists. For
                // copy: do not recurse, follow symlinks, copy file contents.

    // copy_file options:
    skip_existing =
        1u,  // Don't overwrite the existing target file, don't report an error
    overwrite_existing = 1u << 1,  // Overwrite existing file
    update_existing = 1u << 2,     // Overwrite existing file if its last write
                                   // time is older than the replacement file
    synchronize_data = 1u << 3,    // Flush all buffered data written to the
                                   // target file to permanent storage
    synchronize = 1u << 4,  // Flush all buffered data and attributes written to
                            // the target file to permanent storage

    // copy options:
    recursive = 1u << 8,       // Recurse into sub-directories
    copy_symlinks = 1u << 9,   // Copy symlinks as symlinks instead of copying
                               // the referenced file
    skip_symlinks = 1u << 10,  // Don't copy symlinks
    directories_only = 1u << 11,   // Only copy directory structure, do not copy
                                   // non-directory files
    ...
    BOOST_BITMASK(BOOST_SCOPED_ENUM_NATIVE(copy_options))
        ...
        BOOST_SCOPED_ENUM_DECLARE_BEGIN(copy_option){
            ...
            fail_if_exists = none,
            overwrite_if_exists = static_cast<unsigned int>(
                copy_options::
                    overwrite_existing)} BOOST_SCOPED_ENUM_DECLARE_END(copy_option)
#endif
...
#if !defined(BOOST_FILESYSTEM_NO_DEPRECATED)
  BOOST_FILESYSTEM_DECL
  void copy_directory(path const& from, path const& to,
                      system::error_code* ec = 0);
#endif
  BOOST_FILESYSTEM_DECL
  bool copy_file(path const& from, path const& to,  // See ticket #2925
                 unsigned int options,
                 system::error_code* ec = 0);  // see copy_options for options
  BOOST_FILESYSTEM_DECL
  ...
  BOOST_FILESYSTEM_DECL
  bool create_directory(path const& p, const path* existing,
                        system::error_code* ec = 0);
  BOOST_FILESYSTEM_DECL
  void create_directory_symlink(path const& to, path const& from,
                                system::error_code* ec = 0);
  BOOST_FILESYSTEM_DECL
  void create_hard_link(path const& to, path const& from,
                        system::error_code* ec = 0);
  BOOST_FILESYSTEM_DECL
  void create_symlink(path const& to, path const& from,
                      system::error_code* ec = 0);
  BOOST_FILESYSTEM_DECL
  ...
  BOOST_FILESYSTEM_DECL
  path weakly_canonical(path const& p, path const& base,
                        system::error_code* ec = 0);
...
inline file_status status(path const& p) { return detail::status(p); }
...
inline bool exists(path const& p) { return exists(detail::status(p)); }
...
inline bool is_other(path const& p) { return is_other(detail::status(p)); }
...
#ifndef BOOST_FILESYSTEM_NO_DEPRECATED
inline bool is_regular(path const& p) { return is_regular(detail::status(p)); }
...
inline bool is_empty(path const& p) { return detail::is_empty(p); }
...
inline void current_path(path const& p) { detail::current_path(p); }
...
#ifndef BOOST_FILESYSTEM_NO_DEPRECATED
inline path complete(path const& p) { return absolute(p, initial_path()); }
...
inline bool copy_file(path const& from, path const& to) {
  return detail::copy_file(from, to,
                           static_cast<unsigned int>(copy_options::none));
}

inline bool copy_file(path const& from, path const& to,
                      system::error_code& ec) BOOST_NOEXCEPT {
  return detail::copy_file(from, to,
                           static_cast<unsigned int>(copy_options::none), &ec);
}
...
inline void copy_symlink(path const& existing_symlink,
                         path const& new_symlink) {
...
inline path read_symlink(path const& p) { return detail::read_symlink(p); }

inline path read_symlink(path const& p, system::error_code& ec) {
  return detail::read_symlink(p, &ec);
}

inline bool remove(path const& p) { return detail::remove(p); }
...
inline space_info space(path const& p) { return detail::space(p); }
...
inline path weakly_canonical(path const& p, path const& base,
