...
namespace path_detail {  // intentionally don't use filesystem::detail to not
                         // bring internal Boost.Filesystem functions into ADL
                         // via path_constants

template <typename Char, Char Separator, Char PreferredSeparator, Char Dot>
struct path_constants {
  typedef path_constants<Char, Separator, PreferredSeparator, Dot>
      path_constants_base;
...
BOOST_CONSTEXPR_OR_CONST
    typename path_constants<Char, Separator, PreferredSeparator,
                            Dot>::value_type
...
BOOST_CONSTEXPR_OR_CONST
    typename path_constants<Char, Separator, PreferredSeparator,
                            Dot>::value_type
        path_constants<Char, Separator, PreferredSeparator,
                       Dot>::preferred_separator;
template <typename Char, Char Separator, Char PreferredSeparator, Char Dot>
BOOST_CONSTEXPR_OR_CONST
    typename path_constants<Char, Separator, PreferredSeparator,
                            Dot>::value_type
...
// A struct that denotes a contiguous range of characters in a string. A
// lightweight alternative to string_view.
struct substring {
  std::size_t pos;
  std::size_t size;
};
...
struct is_native_char_ptr
    : public is_native_char_ptr_impl<typename boost::remove_cv<
...
template <>
struct is_native_pathable_impl<std::basic_string<wchar_t> >
...
template <>
struct is_native_pathable_impl<std::basic_string<char> >
    : public boost::true_type {};
#endif  // defined(BOOST_WINDOWS_API)
template <>
...
template <typename T>
struct is_native_pathable
    : public is_native_pathable_impl<typename boost::remove_cv<
          typename boost::remove_reference<T>::type>::type> {};
...
class path : public filesystem::path_detail::path_constants<
 ...
 public:
  ...
  //  "const codecvt_type& cvt=codecvt()" default arguments are not used because
  //  this limits the impact of locale("") initialization failures on POSIX
  //  systems to programs that actually depend on locale(""). It further ensures
  //  that exceptions thrown as a result of such failues occur after main() has
  //  started, so can be caught.
  ...
  path(Source const& source,
       typename boost::enable_if_c<
           path_traits::is_pathable<
               typename boost::decay<Source>::type>::value &&
           !path_detail::is_native_pathable<Source>::value>::type* = 0) {
  ...
  path(path&& p, codecvt_type const&) BOOST_NOEXCEPT
  ...
  path(string_type&& s, codecvt_type const&) BOOST_NOEXCEPT
  ...
  path(InputIterator begin, InputIterator end,
       typename boost::disable_if<
           path_detail::is_native_char_ptr<InputIterator> >::type* = 0) {
    if (begin != end) {
  ...
  path(const value_type* begin, const value_type* end, codecvt_type const&)
  ...
  // We need to explicitly define copy assignment as otherwise it will be
  // implicitly defined as deleted because there is move assignment
  path& operator=(path const& p) { return assign(p); }

  path& operator=(string_type const& s) { return assign(s); }

  path& operator=(const value_type* ptr) { return assign(ptr); }

  template <class Source>
  typename boost::enable_if_c<
      path_traits::is_pathable<typename boost::decay<Source>::type>::value &&
          !path_detail::is_native_pathable<Source>::value,
      path&>::type
  ...
  template <class InputIterator>
  typename boost::disable_if<path_detail::is_native_char_ptr<InputIterator>,
                             path&>::type
  ...
  path& operator+=(const value_type* ptr) { return concat(ptr); }

  path& operator+=(string_type const& s) { return concat(s); }
  ...
  typename boost::enable_if<boost::is_integral<CharT>, path&>::type operator+=(
      CharT c) {
    CharT tmp[2];
    tmp[0] = c;
  ...
  path& operator/=(const value_type* ptr) { return append(ptr); }

  path& operator/=(string_type const& s) { return append(s); }
  ...
  BOOST_FORCEINLINE path& append(const value_type* ptr) {
    BOOST_FILESYSTEM_VERSIONED_SYM(append)
    (ptr, ptr + string_type::traits_type::length(ptr));
    return *this;
  }
  ...
  BOOST_FORCEINLINE path& append(const value_type* begin,
  ...
  BOOST_FILESYSTEM_DECL path&
  make_preferred();  // change slashes to backslashes
#endif
  BOOST_FILESYSTEM_DECL path& remove_filename();
  BOOST_FILESYSTEM_DECL path& remove_trailing_separator();
  BOOST_FILESYSTEM_DECL path& replace_extension(
      path const& new_extension = path());
  ...
  std::string string() const {
    std::string tmp;
    if (!m_pathname.empty())
      path_traits::convert(m_pathname.c_str(),
                           m_pathname.c_str() + m_pathname.size(), tmp);
    return tmp;
  }
  std::string string(codecvt_type const& cvt) const {
    ...
    if (!m_pathname.empty())
      path_traits::convert(m_pathname.c_str(),
                           m_pathname.c_str() + m_pathname.size(), tmp, cvt);
    return tmp;
  }
  ...
  std::string generic_string(codecvt_type const& cvt) const {
    return generic_path().string(cvt);
  }
  ...
  std::string const& generic_string(codecvt_type const&) const {
    return m_pathname;
  }
  std::wstring generic_wstring() const { return this->wstring(); }
  std::wstring generic_wstring(codecvt_type const& cvt) const {
    return this->wstring(cvt);
  }
#endif

  //  -----  compare  -----

  BOOST_FORCEINLINE int compare(path const& p) const
      BOOST_NOEXCEPT  // generic, lexicographical
  {
  ...
  path root_path() const {
    return path(m_pathname.c_str(), m_pathname.c_str() + find_root_path_size());
  }
  // returns 0 or 1 element path even on POSIX, root_name() is non-empty() for
  // network paths
  path root_name() const {
    return path(m_pathname.c_str(), m_pathname.c_str() + find_root_name_size());
  }
  ...
  path parent_path() const {
    return path(m_pathname.c_str(),
                m_pathname.c_str() + find_parent_path_size());
  }

  BOOST_FORCEINLINE path filename() const {
    return BOOST_FILESYSTEM_VERSIONED_SYM(filename)();
  }  // returns 0 or 1 element path
  BOOST_FORCEINLINE path stem() const {
    return BOOST_FILESYSTEM_VERSIONED_SYM(stem)();
  }  // returns 0 or 1 element path
  BOOST_FORCEINLINE path extension() const {
    return BOOST_FILESYSTEM_VERSIONED_SYM(extension)();
  }  // returns 0 or 1 element path
  ...
  BOOST_FORCEINLINE bool has_filename() const {
    return BOOST_FILESYSTEM_VERSIONED_SYM(has_filename)();
  }
  ...
  BOOST_FORCEINLINE path lexically_normal() const {
    return BOOST_FILESYSTEM_VERSIONED_SYM(lexically_normal)();
  }
 ...
 private:
  ...
  BOOST_FILESYSTEM_DECL void append_v3(const value_type* b,
                                       const value_type* e);
  BOOST_FILESYSTEM_DECL void append_v4(const value_type* b,
                                       const value_type* e);
  BOOST_FILESYSTEM_DECL void append_v3(path const& p);
  BOOST_FILESYSTEM_DECL void append_v4(path const& p);

  //  Returns: If separator is to be appended, m_pathname.size() before append.
  //  Otherwise 0. Note: An append is never performed if size()==0, so a
  //  returned 0 is unambiguous.
  BOOST_FILESYSTEM_DECL string_type::size_type append_separator_if_needed();
  BOOST_FILESYSTEM_DECL void erase_redundant_separator(
...
namespace detail {
BOOST_FILESYSTEM_DECL int lex_compare_v3(path::iterator first1,
                                         path::iterator last1,
                                         path::iterator first2,
                                         path::iterator last2);
BOOST_FILESYSTEM_DECL int lex_compare_v4(path::iterator first1,
                                         path::iterator last1,
                                         path::iterator first2,
                                         path::iterator last2);
...
class path::iterator
 ...
 private:
  ...
  friend BOOST_FILESYSTEM_DECL int detail::lex_compare_v3(path::iterator first1,
                                                          path::iterator last1,
                                                          path::iterator first2,
                                                          path::iterator last2);
  ...
  BOOST_FORCEINLINE void increment() {
    BOOST_FILESYSTEM_VERSIONED_SYM(increment)();
  }
  BOOST_FORCEINLINE void decrement() {
    BOOST_FILESYSTEM_VERSIONED_SYM(decrement)();
  }
...
class path::reverse_iterator
    : public boost::iterator_facade<path::reverse_iterator, const path,
                                    boost::bidirectional_traversal_tag> {
 public:
  explicit reverse_iterator(iterator itr) : m_itr(itr) {
    if (itr != itr.m_path_ptr->begin()) m_element = *--itr;
  }
...
inline bool lexicographical_compare(path::iterator first1, path::iterator last1,
                                    path::iterator first2,
                                    path::iterator last2) {
  return BOOST_FILESYSTEM_VERSIONED_SYM(detail::lex_compare)(first1, last1,
                                                             first2, last2) < 0;
}
...
// Note: Declared as a template to delay binding to Boost.ContainerHash
// functions and make the dependency optional
template <typename T>
inline typename boost::enable_if<boost::is_same<T, path>, std::size_t>::type
hash_value(T const& p) BOOST_NOEXCEPT {
#ifdef BOOST_WINDOWS_API
  std::size_t seed = 0;
...
inline std::basic_ostream<Char, Traits>& operator<<(
    std::basic_ostream<Char, Traits>& os, path const& p) {
...
inline std::basic_istream<Char, Traits>& operator>>(
    std::basic_istream<Char, Traits>& is, path& p) {
...
template <>
inline std::string path::generic_string<std::string>(
    codecvt_type const& cvt) const {
  return generic_string(cvt);
}

template <>
inline std::wstring path::generic_string<std::wstring>(
    codecvt_type const& cvt) const {
