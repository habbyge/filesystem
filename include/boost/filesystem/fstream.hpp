...
#if (defined(_CPPLIB_VER) && _CPPLIB_VER >= 405 &&          \
     !defined(_STLPORT_VERSION)) ||                         \
    (defined(_LIBCPP_VERSION) && _LIBCPP_VERSION >= 7000 && \
     defined(_LIBCPP_HAS_OPEN_WITH_WCHAR))
...
// - std::filesystem is available in gcc 8 but it is broken there (fails to
// compile path definition
//   on Windows). Compilation errors seem to be fixed since gcc 9.
// - In gcc 10.2 and clang 8.0.1 on Cygwin64, the path attempts to convert the
// wide string to narrow
//   and fails in runtime. This may be system locale dependent, and performing
//   character code conversion is against the purpose of using
//   std::filesystem::path anyway.
// - Other std::filesystem implementations were not tested, so it is not known
...
#pragma warning(push)
// 'boost::filesystem::basic_fstream<charT>' : inherits
...
//                                  basic_filebuf //
//--------------------------------------------------------------------------------------//
// std::basic_filebuf 是关联字符序列为文件的 std::basic_streambuf 。输入序列和输出序列都关联到同一文件，
// 并为两种操作维护连接文件位置。详见：https://www.apiref.com/cpp-zh/cpp/io/basic_filebuf.html
// 函数 underflow() 和 overflow()/sync() 进行文件和缓冲区的获取放置区之间的实际 I/O 。
// charT 不是 char 时，多数实现在文件存储多字节字符，并用 std::codecvt 刻面进行宽/多字节字符转换。
...
class basic_ifstream : public std::basic_ifstream<charT, traits> {
 public:
  ...
  explicit basic_ifstream(path const& p)
      : std::basic_ifstream<charT, traits>(BOOST_FILESYSTEM_C_STR(p),
                                           std::ios_base::in) {}
...
class basic_ofstream : public std::basic_ofstream<charT, traits> {
 public:
  ...
  explicit basic_ofstream(path const& p)
      : std::basic_ofstream<charT, traits>(BOOST_FILESYSTEM_C_STR(p),
                                           std::ios_base::out) {}
 ...
 public:
  void open(path const& p) {
    std::basic_ofstream<charT, traits>::open(BOOST_FILESYSTEM_C_STR(p),
                                             std::ios_base::out);
  }
 ...
 public:
  ...
  explicit basic_fstream(path const& p)
      : std::basic_fstream<charT, traits>(
            BOOST_FILESYSTEM_C_STR(p), std::ios_base::in | std::ios_base::out) {
  }
