...
class file_status {
 public:
  BOOST_CONSTEXPR file_status() BOOST_NOEXCEPT : m_value(status_error),
  ...
  BOOST_CONSTEXPR file_status(const file_status& rhs) BOOST_NOEXCEPT
      : m_value(rhs.m_value),
        m_perms(rhs.m_perms) {}
  BOOST_CXX14_CONSTEXPR file_status& operator=(const file_status& rhs)
      BOOST_NOEXCEPT {
    m_value = rhs.m_value;
    m_perms = rhs.m_perms;
    return *this;
  ...
  // Note: std::move is not constexpr in C++11, that's why we're not using it
  // here
  ...
  BOOST_CXX14_CONSTEXPR file_status& operator=(file_status&& rhs)
      BOOST_NOEXCEPT {
    m_value = static_cast<file_type&&>(rhs.m_value);
    m_perms = static_cast<enum perms&&>(rhs.m_perms);
    return *this;
  ...
  BOOST_CXX14_CONSTEXPR void permissions(perms prms) BOOST_NOEXCEPT {
    m_perms = prms;
  }
