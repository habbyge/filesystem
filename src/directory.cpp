...
// TODO: find out what macros indicate dirent::d_type present in more libraries
    ...
    defined(                      \
        _DIRENT_HAVE_D_TYPE)  // defined by GNU C library if d_type present
...
inline std::size_t get_path_max() {
  ...
  if ((max + 1) < sizeof(dirent().d_name)) max = sizeof(dirent().d_name) - 1;
...
error_code dir_itr_first(void*& handle, void*& buffer, const char* dir,
                         std::string& target, fs::file_status&,
                         fs::file_status&) {
...
inline int readdir_r_simulator(DIR* dirp, void*& buffer,
  ...
  if (::sysconf(_SC_THREAD_SAFE_FUNCTIONS) >= 0) {
    ...
    if (BOOST_UNLIKELY(!storage)) {
      // According to readdir description, there's no reliable way to predict
      // the length of the d_name string. It may exceed NAME_MAX and
      // pathconf(_PC_NAME_MAX) limits. We are being conservative here and
      // allocate buffer that is enough for PATH_MAX as the directory name.
      // Still, this doesn't guarantee there won't be a buffer overrun. The
      // readdir_r API is fundamentally flawed and we should avoid it as much as
      // possible in favor of readdir.
      const std::size_t name_size = path_max();
      const std::size_t buffer_size =
          (sizeof(dirent) - sizeof(dirent().d_name)) + name_size +
          1;  // + 1 for "\0"
...
error_code dir_itr_increment(void*& handle, void*& buffer, std::string& target,
                             fs::file_status& sf, fs::file_status& symlink_sf) {
  dirent* result = NULL;
  int err = readdir_r_simulator(static_cast<DIR*>(handle), buffer, &result);
  if (BOOST_UNLIKELY(err != 0)) return error_code(err, system_category());
  if (result == NULL) return fs::detail::dir_itr_close(handle, buffer);
...
error_code dir_itr_first(void*& handle, fs::path const& dir,
                         std::wstring& target, fs::file_status& sf,
                         fs::file_status& symlink_sf)
...
// error. It is treated as eof instead.
{
  ...
  // reparse points are complex, so don't try to handle them here; instead just
  // mark them as status_error which causes directory_entry caching to call
  // status() and symlink_status() which do handle reparse points fully
  {
...
error_code dir_itr_increment(void*& handle, std::wstring& target,
                             fs::file_status& sf, fs::file_status& symlink_sf) {
  WIN32_FIND_DATAW data;
...
BOOST_FILESYSTEM_DECL
void directory_iterator_construct(directory_iterator& it, path const& p,
                                  unsigned int opts, system::error_code* ec) {
  ...
  try {
    ...
    if (result) {
      if (result != make_error_condition(system::errc::permission_denied) ||
          (opts & static_cast<unsigned int>(
                      directory_options::skip_permission_denied)) == 0u) {
    ...
    if (imp->handle) {
      ...
      if (filename_str[0] == path::dot  // dot or dot-dot
          && (filename_str[1] ==
                  static_cast<path::string_type::value_type>('\0') ||
              (filename_str[1] == path::dot &&
               filename_str[2] ==
                   static_cast<path::string_type::value_type>('\0')))) {
...
void directory_iterator_increment(directory_iterator& it,
  ...
  try {
    ...
    for (;;) {
      ...
      if (BOOST_UNLIKELY(!!increment_ec))  // happens if filesystem is corrupt,
                                           // such as on a damaged optical disc
      {
        boost::intrusive_ptr<detail::dir_itr_imp> imp;
        imp.swap(it.m_imp);
        path error_path(
            imp->dir_entry.path().parent_path());  // fix ticket #5900
        if (!ec)
          BOOST_FILESYSTEM_THROW(filesystem_error(
              "boost::filesystem::directory_iterator::operator++", error_path,
              increment_ec));
...
BOOST_FILESYSTEM_DECL
void recursive_directory_iterator_construct(recursive_directory_iterator& it,
                                            path const& dir_path,
                                            unsigned int opts,
                                            system::error_code* ec) {
  if (ec) ec->clear();
  ...
  if ((ec && *ec) || dir_it == directory_iterator()) return;
...
void recursive_directory_iterator_pop_on_error(detail::recur_dir_itr_imp* imp) {
  ...
  while (!imp->m_stack.empty()) {
    ...
    if (!increment_ec && dir_it != directory_iterator()) break;
...
BOOST_FILESYSTEM_DECL
void recursive_directory_iterator_pop(recursive_directory_iterator& it,
                                      system::error_code* ec) {
  ...
  while (true) {
    ...
    if (BOOST_UNLIKELY(!!increment_ec)) {
      if ((imp->m_options &
           static_cast<unsigned int>(directory_options::pop_on_error)) == 0u) {
    ...
    if (dir_it != directory_iterator()) break;
...
inline push_directory_result recursive_directory_iterator_push_directory(
    detail::recur_dir_itr_imp* imp, system::error_code& ec) BOOST_NOEXCEPT {
  ...
  try {
    ...
    if ((imp->m_options &
         static_cast<unsigned int>(directory_options::_detail_no_push)) != 0u) {
      imp->m_options &=
    ...
    // occurred
    if ((imp->m_options & static_cast<unsigned int>(
                              directory_options::follow_directory_symlink)) ==
            0u ||
        (imp->m_options & static_cast<unsigned int>(
    ...
    if ((imp->m_options & static_cast<unsigned int>(
        ...
        if (ec ==
            ...
            (imp->m_options & static_cast<unsigned int>(
                ...
                static_cast<unsigned int>(
                    directory_options::follow_directory_symlink |
                    directory_options::skip_dangling_symlinks)) {
          // Skip dangling symlink and continue iteration on the current depth
          // level
          ec = error_code();
        }
      ...
      if (!fs::is_directory(stat)) return result;
      ...
      if (BOOST_UNLIKELY(
        ...
        // When depth overflow happens, avoid popping the current directory
        // iterator and attempt to continue iteration on the current depth.
        result = keep_depth;
        return result;
      }

      directory_iterator next(
          imp->m_stack.back()->path(),
          static_cast<BOOST_SCOPED_ENUM_NATIVE(directory_options)>(
              imp->m_options),
          ec);
...
BOOST_FILESYSTEM_DECL
void recursive_directory_iterator_increment(recursive_directory_iterator& it,
                                            system::error_code* ec) {
  BOOST_ASSERT_MSG(!it.is_end(),
  ...
  //  if various conditions are met, push a directory_iterator into the iterator
  //  stack
  push_directory_result push_result =
      recursive_directory_iterator_push_directory(imp, local_ec);
  if (push_result == directory_pushed) return;
  ...
  on_error:
    if ((imp->m_options &
      ...
      if (imp->m_stack.empty()) it.m_imp.reset();  // done, so make end iterator
    }
  ...
  //  stack, popping the stack if necessary, until either the stack is empty or
  //  a non-end iterator is reached.
  while (true) {
    if (imp->m_stack.empty()) {
      it.m_imp.reset();  // done, so make end iterator
      break;
    ...
    if (BOOST_UNLIKELY(!!local_ec)) goto on_error;
