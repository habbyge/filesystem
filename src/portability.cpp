...
BOOST_FILESYSTEM_DECL bool portable_posix_name(std::string const& name) {
  return !name.empty() && name.find_first_not_of(posix_valid_chars, 0,
                                                 sizeof(posix_valid_chars) -
                                                     1) == std::string::npos;
}

BOOST_FILESYSTEM_DECL bool windows_name(std::string const& name) {
  // note that the terminating '\0' is part of the string - thus the size below
  // is sizeof(windows_invalid_chars) rather than
  // sizeof(windows_invalid_chars)-1.
  return !name.empty() && name[0] != ' ' &&
         name.find_first_of(windows_invalid_chars, 0,
                            sizeof(windows_invalid_chars)) ==
             std::string::npos &&
         *(name.end() - 1) != ' ' &&
         (*(name.end() - 1) != '.' || name.size() == 1 || name == "..");
}
