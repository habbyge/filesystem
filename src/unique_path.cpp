...
#include <unistd.h>
    ...
    BOOST_OS_BSD_FREE >= BOOST_VERSION_NUMBER(8, 0, 0) || BOOST_LIB_C_CLOUDABI
#include <stdlib.h>
...
// We use auto-linking below to help users of static builds of Boost.Filesystem
// to link to whatever Windows SDK library we selected. The dependency
// information is currently not exposed in CMake config files generated by
// Boost.Build (https://github.com/boostorg/boost_install/issues/18), which
// makes it non-trivial for users to discover the libraries they need. This
// feature is deprecated and may be removed in the future, when the situation
// with CMake config files improves. Note that the library build system is the
// principal source of linking the library, which must work regardless of
// auto-linking.
#include <boost/predef/platform.h>

#include <boost/winapi/basic_types.hpp>

#if defined(BOOST_FILESYSTEM_HAS_BCRYPT)  // defined on the command line by the
                                          // project
...
inline boost::winapi::DWORD_ translate_ntstatus(
    boost::winapi::NTSTATUS_ status) {
  // Note: Legacy MinGW doesn't have ntstatus.h and doesn't define NTSTATUS
  // error codes other than STATUS_SUCCESS.
  //       Because of this we have to use hardcoded integer literals here. Also,
  //       we have to cast to unsigned integral type to avoid signed overflow
  //       and narrowing conversion in the constants.
...
#if defined(BOOST_FILESYSTEM_HAS_GETRANDOM) || \
    defined(BOOST_FILESYSTEM_HAS_GETRANDOM_SYSCALL)
  ...
  while (bytes_read < len) {
    ...
    if (BOOST_UNLIKELY(n < 0)) {
      ...
      if (err == ENOSYS && bytes_read == 0u) {
        filesystem::detail::atomic_store_relaxed(fill_random,
...
#endif  // defined(BOOST_FILESYSTEM_HAS_GETRANDOM) ||
        // defined(BOOST_FILESYSTEM_HAS_GETRANDOM_SYSCALL)
...
void system_crypt_random(void* buf, std::size_t len,
  ...
  boost::winapi::NTSTATUS_ status = boost::winapi::BCryptOpenAlgorithmProvider(
      &handle, boost::winapi::BCRYPT_RNG_ALGORITHM_, NULL, 0);
  if (BOOST_UNLIKELY(status != 0)) {
  fail:
    emit_error(translate_ntstatus(status), ec,
               "boost::filesystem::unique_path");
    return;
  }

  status = boost::winapi::BCryptGenRandom(
      handle, static_cast<boost::winapi::PUCHAR_>(buf),
      static_cast<boost::winapi::ULONG_>(len), 0);

  boost::winapi::BCryptCloseAlgorithmProvider(handle, 0);

  if (BOOST_UNLIKELY(status != 0)) goto fail;

#else  // defined(BOOST_FILESYSTEM_HAS_BCRYPT)

  boost::winapi::HCRYPTPROV_ handle;
  boost::winapi::DWORD_ err = 0u;
  if (BOOST_UNLIKELY(!boost::winapi::CryptAcquireContextW(
          &handle, NULL, NULL, boost::winapi::PROV_RSA_FULL_,
  ...
  boost::winapi::BOOL_ gen_ok = boost::winapi::CryptGenRandom(
      handle, static_cast<boost::winapi::DWORD_>(len),
      static_cast<boost::winapi::BYTE_*>(buf));

  if (BOOST_UNLIKELY(!gen_ok)) err = boost::winapi::GetLastError();

  boost::winapi::CryptReleaseContext(handle, 0);

  if (BOOST_UNLIKELY(!gen_ok)) goto fail;
...
void init_fill_random_impl(unsigned int major_ver, unsigned int minor_ver,
    ...
    (defined(BOOST_FILESYSTEM_HAS_GETRANDOM) ||    \
     defined(BOOST_FILESYSTEM_HAS_GETRANDOM_SYSCALL))
...
path unique_path(path const& model, system::error_code* ec) {
  ...
  char ran[16] = {};  // init to avoid clang static analyzer message
                      // see ticket #8954
  BOOST_CONSTEXPR_OR_CONST unsigned int max_nibbles =
      2u * sizeof(ran);  // 4-bits per nibble
