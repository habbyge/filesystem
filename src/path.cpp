...
//! Returns position of the first directory separator in the \a size initial
//! characters of \a p, or \a size if not found
inline size_type find_separator(const wchar_t* p,
                                size_type size) BOOST_NOEXCEPT {
  size_type pos = 0u;
  for (; pos < size; ++pos) {
    const wchar_t c = p[pos];
    if (boost::filesystem::detail::is_directory_separator(c)) break;
  }
  return pos;
}
...
bool is_root_separator(string_type const& str, size_type root_dir_pos,
                       size_type pos);

// Returns: Size of the filename element that ends at end_pos (which is
// past-the-end position). 0 if no filename found.
size_type find_filename_size(string_type const& str, size_type root_name_size,
                             size_type end_pos);

// Returns: starting position of root directory or size if not found. Sets
// root_name_size to length of the root name if the characters before the
// returned position (if any) are considered a root name.
size_type find_root_directory_start(const value_type* path, size_type size,
                                    size_type& root_name_size);

// Finds position and size of the first element of the path
void first_element(string_type const& src, size_type& element_pos,
                   size_type& element_size, size_type size);

// Finds position and size of the first element of the path
inline void first_element(string_type const& src, size_type& element_pos,
...
BOOST_FILESYSTEM_DECL void path::append_v3(const value_type* begin,
                                           const value_type* end) {
  if (begin != end) {
    if (BOOST_LIKELY(begin < m_pathname.data() ||
                     begin >= (m_pathname.data() + m_pathname.size()))) {
      if (!detail::is_directory_separator(*begin)) append_separator_if_needed();
...
BOOST_FILESYSTEM_DECL void path::append_v4(path const& p) {
  if (!p.empty()) {
    if (BOOST_LIKELY(this != &p)) {
      ...
      size_type that_root_dir_pos = find_root_directory_start(
          p.m_pathname.c_str(), that_size, that_root_name_size);
      ...
      find_root_directory_start(m_pathname.c_str(), m_pathname.size(),
                                this_root_name_size);

      if (that_root_name_size > 0 &&
          (that_root_name_size != this_root_name_size ||
           std::memcmp(m_pathname.c_str(), p.m_pathname.c_str(),
                       this_root_name_size * sizeof(value_type)) != 0)) {
        goto return_assign;
      }

      if (that_root_dir_pos < that_size) {
        // Remove root directory (if any) and relative path to replace with
        // those from p
        m_pathname.erase(m_pathname.begin() + this_root_name_size,
                         m_pathname.end());
      }

      const value_type* const that_path =
          p.m_pathname.c_str() + that_root_name_size;
...
BOOST_FILESYSTEM_DECL void path::append_v4(const value_type* begin,
                                           const value_type* end) {
  if (begin != end) {
    if (BOOST_LIKELY(begin < m_pathname.data() ||
                     begin >= (m_pathname.data() + m_pathname.size()))) {
      ...
      size_type that_root_name_size = 0;
      size_type that_root_dir_pos =
          find_root_directory_start(begin, that_size, that_root_name_size);
      ...
      if (that_root_name_size > 0 &&
          (that_root_name_size != this_root_name_size ||
           std::memcmp(m_pathname.c_str(), begin,
                       this_root_name_size * sizeof(value_type)) != 0)) {
        goto return_assign;
      }
...
BOOST_FILESYSTEM_DECL void path::erase_redundant_separator(
    string_type::size_type sep_pos) {
  if (sep_pos                                   // a separator was added
    ...
    m_pathname.erase(m_pathname.begin() +
                     sep_pos);  // erase the added separator
  }
}
...
BOOST_FILESYSTEM_DECL path& path::replace_extension(path const& new_extension) {
  ...
  if (!new_extension.empty()) {
    ...
    if (new_extension.m_pathname[0] != dot) m_pathname.push_back(dot);
...
BOOST_FILESYSTEM_DECL size_type path::find_root_path_size() const {
  size_type root_name_size = 0;
  size_type root_dir_pos = find_root_directory_start(
      m_pathname.c_str(), m_pathname.size(), root_name_size);

  size_type size = root_name_size;
  if (root_dir_pos < m_pathname.size()) size = root_dir_pos + 1;
...
BOOST_FILESYSTEM_DECL substring path::find_root_directory() const {
  substring root_dir;
  size_type root_name_size = 0;
  root_dir.pos = find_root_directory_start(m_pathname.c_str(),
                                           m_pathname.size(), root_name_size);
...
BOOST_FILESYSTEM_DECL substring path::find_relative_path() const {
  ...
  if (root_dir_pos < m_pathname.size()) {
    ...
    for (size_type n = m_pathname.size(); size < n; ++size) {
      if (!detail::is_directory_separator(m_pathname[size])) break;
    }
  }
...
BOOST_FILESYSTEM_DECL string_type::size_type path::find_parent_path_size()
    const {
  ...
  size_type filename_size =
      find_filename_size(m_pathname, root_name_size, size);
...
BOOST_FILESYSTEM_DECL path path::filename_v3() const {
  ...
  if (root_dir_pos < size &&
      detail::is_directory_separator(m_pathname[size - 1]) &&
      is_root_separator(m_pathname, root_dir_pos, size - 1)) {
    ...
    if (filename_size == 0u && pos > root_name_size &&
        detail::is_directory_separator(m_pathname[pos - 1]) &&
        !is_root_separator(m_pathname, root_dir_pos, pos - 1))
...
BOOST_FILESYSTEM_DECL string_type::size_type path::find_filename_v4_size()
    const {
  const size_type size = m_pathname.size();
  size_type root_name_size = 0;
  find_root_directory_start(m_pathname.c_str(), size, root_name_size);
...
BOOST_FILESYSTEM_DECL path path::stem_v3() const {
  ...
  if (name != detail::dot_path() && name != detail::dot_dot_path()) {
    ...
    if (pos != string_type::npos)
      name.m_pathname.erase(name.m_pathname.begin() + pos,
                            name.m_pathname.end());
  }
  return name;
}
...
BOOST_FILESYSTEM_DECL path path::extension_v4() const {
  ...
  if (filename_size > 0u &&
      ...
      !(m_pathname[filename_pos] == dot &&
        (filename_size == 1u ||
         (filename_size == 2u && m_pathname[filename_pos + 1u] == dot)))) {
    size_type ext_pos = size;
    while (ext_pos > filename_pos) {
      --ext_pos;
      if (m_pathname[ext_pos] == dot) break;
    }
...
namespace detail {
// C++14 provides a mismatch algorithm with four iterator arguments(), but
// earlier standard libraries didn't, so provide this needed functionality.
inline std::pair<path::iterator, path::iterator> mismatch(
    path::iterator it1, path::iterator it1end, path::iterator it2,
...
BOOST_FILESYSTEM_DECL path path::lexically_relative(path const& base) const {
  path::iterator b = begin(), e = end(), base_b = base.begin(),
                 base_e = base.end();
  std::pair<path::iterator, path::iterator> mm =
      detail::mismatch(b, e, base_b, base_e);
  if (mm.first == b && mm.second == base_b) return path();
  if (mm.first == e && mm.second == base_e) return detail::dot_path();
  ...
  if (n < 0) return path();
  if (n == 0 && (mm.first == e || mm.first->empty())) return detail::dot_path();

  path tmp;
  for (; n > 0; --n) tmp /= detail::dot_dot_path();
  for (; mm.first != e; ++mm.first) tmp /= *mm.first;
  return tmp;
}
...
BOOST_FILESYSTEM_DECL path path::lexically_normal_v3() const {
  ...
  while (i < pathname_size && detail::is_directory_separator(pathname[i])) ++i;
    ...
    while (true) {
      {
        ...
        if (size == 2u && pathname[start_pos] == dot &&
            pathname[start_pos + 1] == dot &&
            normal.m_pathname.size() > root_path_size) {
          // Don't remove previous dot dot elements
          const size_type normal_size = normal.m_pathname.size();
          size_type filename_size = find_filename_size(
              normal.m_pathname, root_path_size, normal_size);
          size_type pos = normal_size - filename_size;
          if (filename_size != 2u || normal.m_pathname[pos] != dot ||
              normal.m_pathname[pos + 1] != dot) {
            if (pos > root_path_size &&
                detail::is_directory_separator(normal.m_pathname[pos - 1]))
              --pos;
            normal.m_pathname.erase(normal.m_pathname.begin() + pos,
                                    normal.m_pathname.end());
            goto skip_append;
          }
        }
...
BOOST_FILESYSTEM_DECL path path::lexically_normal_v4() const {
  ...
  if (i < pathname_size) {
    ...
    skip_append:
      if (i == pathname_size) {
        // If a path ends with a trailing dot after a directory element, add a
        // trailing separator
        if (last_element_was_dot && !normal.empty() &&
            !normal.filename_is_dot_dot())
          normal.append_separator_if_needed();
    ...
    // If the original path was not empty and normalized ended up being empty,
    // make it a dot
    if (normal.empty()) normal.m_pathname.push_back(dot);
  }
...
// pos is position of the separator
inline bool is_root_separator(string_type const& str, size_type root_dir_pos,
                              size_type pos) {
  BOOST_ASSERT_MSG(
      pos < str.size() && fs::detail::is_directory_separator(str[pos]),
      "precondition violation");

  // root_dir_pos points at the leftmost separator, we need to skip any
  // duplicate separators right of root dir
...
// Returns: Size of the filename element that ends at end_pos (which is
// past-the-end position). 0 if no filename found.
inline size_type find_filename_size(string_type const& str,
                                    size_type root_name_size,
                                    size_type end_pos) {
...
size_type find_root_directory_start(const value_type* path, size_type size,
  ...
  if (fs::detail::is_directory_separator(path[0])) {
    if (size >= 2 && fs::detail::is_directory_separator(path[1])) {
      ...
      // cases "\\?\" and "\\.\"
               ...
               (path[2] == questionmark || path[2] == fs::path::dot) &&
        ...
        // The path starts with three directory separators, which is interpreted
    ...
    // case "\??\" (NT path prefix)
    else if (size >= 4 && path[1] == questionmark && path[2] == questionmark &&
             fs::detail::is_directory_separator(path[3])) {
      parsing_root_name = true;
      pos += 4;
    }
      ...
      // The path starts with a separator, possibly followed by a non-separator
      // character
      return 0;
    }
  }
...
#ifdef BOOST_WINDOWS_API
  // case "c:" or "prn:"
  // Note: There is ambiguity in a "c:x" path interpretation. It could either
  // mean a file "x" located at the current directory for drive C:,
  //       or an alternative stream "x" of a file "c". Windows API resolve this
  //       as the former, and so do we.
  if ((size - pos) >= 2 && fs::detail::is_letter(path[pos])) {
    size_type i = pos + 1;
    for (; i < size; ++i) {
      if (!is_device_name_char(path[i])) break;
    }
  ...
  if (!parsing_root_name) return size;

find_next_separator:
  pos += find_separator(path + pos, size - pos);
  if (parsing_root_name) root_name_size = pos;
...
void first_element(string_type const& src, size_type& element_pos,
  ...
  size_type root_name_size = 0;
  size_type root_dir_pos =
      find_root_directory_start(src.c_str(), size, root_name_size);
  ...
  if (end_pos == string_type::npos) end_pos = src.size();
...
int lex_compare_v3(path::iterator first1, path::iterator last1,
...
BOOST_FILESYSTEM_DECL
int lex_compare_v4(path::iterator first1, path::iterator last1,
                   path::iterator first2, path::iterator last2) {
  for (; first1 != last1 && first2 != last2;) {
    if (first1->native() < first2->native()) return -1;
    if (first2->native() < first1->native()) return 1;
...
#ifdef BOOST_WINDOWS_API
    if (itr.m_element.m_pathname.size() == 1u &&
        itr.m_element.m_pathname[0] == preferred_separator)
...
BOOST_FILESYSTEM_DECL void path::iterator::increment_v3() {
  ...
  // increment to position past current element; if current element is implicit
  // dot, this will cause m_pos to represent the end iterator
  ...
  if (m_pos >= size) {
    BOOST_ASSERT_MSG(
        m_pos == size,
        "path::iterator increment after the referenced path was modified");
  ...
  if (detail::is_directory_separator(m_path_ptr->m_pathname[m_pos])) {
    size_type root_name_size = 0;
    size_type root_dir_pos = find_root_directory_start(
        m_path_ptr->m_pathname.c_str(), size, root_name_size);

    // detect root directory and set iterator value to the separator if it is
    if (m_pos == root_dir_pos &&
        m_element.m_pathname.size() == root_name_size) {
    ...
    while (m_pos != size &&
           detail::is_directory_separator(m_path_ptr->m_pathname[m_pos])) {
      ++m_pos;
    }
  ...
  if (end_pos == string_type::npos) end_pos = size;
...
BOOST_FILESYSTEM_DECL void path::iterator::increment_v4() {
  ...
  if (m_element.m_pathname.empty() && (m_pos + 1) == size &&
      detail::is_directory_separator(m_path_ptr->m_pathname[m_pos])) {
    // The iterator was pointing to the last empty element of the path; set to
    // end.
    m_pos = size;
    return;
  }
...
BOOST_FILESYSTEM_DECL void path::iterator::decrement_v3() {
  ...
  BOOST_ASSERT_MSG(
      m_pos <= size,
      "path::iterator decrement after the referenced path was modified");
  ...
  if (end_pos <= root_name_size) goto set_to_root_name;

  size_type filename_size =
      find_filename_size(m_path_ptr->m_pathname, root_name_size, end_pos);
...
std::locale default_locale() {
  ...
  return std::locale(global_loc,
                     new boost::filesystem::detail::windows_file_codecvt());
#elif defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__) || \
    defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__HAIKU__)
  ...
  // when using HFS. The right way to deal with it would be to always convert
  // the filename to UTF-8 before trying to open/create a file." See
  ...
  // it's stored on disk depends on the disk format; HFS+ uses UTF-16, but
  // that's not important in most cases." See
...
// g_path_locale is only initialized if path::codecvt() or path::imbue() are
...
inline std::locale& get_path_locale() {
  ...
  if (BOOST_UNLIKELY(!p)) {
    ...
    if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel,
                                  atomic_ns::memory_order_acquire)) {
      p = new_p;
...
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
  std::locale* p = atomic_ns::atomic_ref<std::locale*>(g_path_locale)
                       .exchange(new_p, atomic_ns::memory_order_acq_rel);
#else
  std::locale* p = g_path_locale;
  g_path_locale = new_p;
#endif
  if (!p) schedule_path_locale_cleanup();
  return p;
}
...
inline boost::filesystem::path const& get_dot_path() {
  ...
  if (BOOST_UNLIKELY(!p)) {
    const boost::filesystem::path* new_p =
        new boost::filesystem::path(dot_path_literal);
    if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel,
                                  atomic_ns::memory_order_acquire))
      p = new_p;
    else
...
inline boost::filesystem::path const& get_dot_dot_path() {
  ...
  if (BOOST_UNLIKELY(!p)) {
    const boost::filesystem::path* new_p =
        new boost::filesystem::path(dot_dot_path_literal);
    if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel,
                                  atomic_ns::memory_order_acquire))
      p = new_p;
    else
...
BOOST_ATTRIBUTE_UNUSED
    BOOST_FILESYSTEM_ATTRIBUTE_RETAIN extern const init_func_ptr_t
        p_init_path_globals = &init_path_globals;
#pragma data_seg()
#if (_MSC_VER >= 1300)  // 1300 == VC++ 7.0
...
BOOST_ATTRIBUTE_UNUSED const path_locale_deleter g_path_locale_deleter = {};
BOOST_FILESYSTEM_INIT_PRIORITY(BOOST_FILESYSTEM_PATH_GLOBALS_INIT_PRIORITY)
...
inline boost::filesystem::path const& get_dot_path() { return g_dot_path; }
...
BOOST_FILESYSTEM_DECL path const& dot_path() { return get_dot_path(); }

BOOST_FILESYSTEM_DECL path const& dot_dot_path() { return get_dot_dot_path(); }
