...
// WASI does not have statfs or statvfs.
    ...
    (!defined(__OpenBSD__) ||                               \
     BOOST_OS_BSD_OPEN >= BOOST_VERSION_NUMBER(4, 4, 0)) && \
    !defined(__ANDROID__) && !defined(__VXWORKS__)
...
#if !defined(BOOST_FILESYSTEM_DISABLE_COPY_FILE_RANGE) && \
    defined(__NR_copy_file_range)
#define BOOST_FILESYSTEM_USE_COPY_FILE_RANGE
#endif  // !defined(BOOST_FILESYSTEM_DISABLE_COPY_FILE_RANGE) &&
        // defined(__NR_copy_file_range)
#if !defined(BOOST_FILESYSTEM_DISABLE_STATX) && \
    (defined(BOOST_FILESYSTEM_HAS_STATX) ||     \
     defined(BOOST_FILESYSTEM_HAS_STATX_SYSCALL))
#if !defined(BOOST_FILESYSTEM_HAS_STATX) && \
    defined(BOOST_FILESYSTEM_HAS_STATX_SYSCALL)
#include <linux/stat.h>
#endif
#define BOOST_FILESYSTEM_USE_STATX
#endif  // !defined(BOOST_FILESYSTEM_DISABLE_STATX) &&
        // (defined(BOOST_FILESYSTEM_HAS_STATX) ||
        // defined(BOOST_FILESYSTEM_HAS_STATX_SYSCALL))
...
// Our convenience type for allocating REPARSE_DATA_BUFFER along with sufficient
// space after it
...
#if defined(linux) || defined(__linux) || defined(__linux__)
//! Initializes fill_random implementation pointer. Implemented in
//! unique_path.cpp.
void init_fill_random_impl(unsigned int major_ver, unsigned int minor_ver,
...
// Size of a small buffer for a path that can be placed on stack, in character
// code units
BOOST_CONSTEXPR_OR_CONST std::size_t small_path_size = 1024u;

// Absolute maximum path length, in character code units, that we're willing to
// accept from various system calls. This value is arbitrary, it is supposed to
// be a hard limit to avoid memory exhaustion in some of the algorithms below in
// case of some corrupted or maliciously broken filesystem. A few examples of
// path size limits:
...
bool is_empty_directory(path const& p, error_code* ec) {
  ...
  detail::directory_iterator_construct(
      itr, p, static_cast<unsigned int>(directory_options::none), ec);
...
//! A wrapper for statx libc function. Disable MSAN since at least on clang 10
//! it doesn't know which fields of struct statx are initialized by the syscall
//! and misdetects errors.
BOOST_FILESYSTEM_NO_SANITIZE_MEMORY
BOOST_FORCEINLINE int invoke_statx(int dirfd, const char* path, int flags,
...
int statx_fstatat(int dirfd, const char* path, int flags, unsigned int mask,
                  struct ::statx* stx) {
  struct ::stat st;
...
typedef int statx_t(int dirfd, const char* path, int flags, unsigned int mask,
                    struct ::statx* stx);

//! Pointer to the actual implementation of the statx implementation
statx_t* statx_ptr = &statx_fstatat;

inline int invoke_statx(int dirfd, const char* path, int flags,
                        unsigned int mask, struct ::statx* stx) BOOST_NOEXCEPT {
  return filesystem::detail::atomic_load_relaxed(statx_ptr)(dirfd, path, flags,
                                                            mask, stx);
}

//! A wrapper for the statx syscall. Disable MSAN since at least on clang 10 it
//! doesn't know which fields of struct statx are initialized by the syscall and
//! misdetects errors.
BOOST_FILESYSTEM_NO_SANITIZE_MEMORY
int statx_syscall(int dirfd, const char* path, int flags, unsigned int mask,
                  struct ::statx* stx) {
...
inline void init_statx_impl(unsigned int major_ver, unsigned int minor_ver,
                            unsigned int patch_ver) {
#if !defined(BOOST_FILESYSTEM_HAS_STATX) && \
    defined(BOOST_FILESYSTEM_HAS_STATX_SYSCALL)
...
inline bool equivalent_stat(struct ::statx const& s1,
                            struct ::statx const& s2) BOOST_NOEXCEPT {
  return s1.stx_dev_major == s2.stx_dev_major &&
         s1.stx_dev_minor == s2.stx_dev_minor && s1.stx_ino == s2.stx_ino;
}
...
inline bool equivalent_stat(struct ::stat const& s1,
                            struct ::stat const& s2) BOOST_NOEXCEPT {
...
//! Flushes buffered data and attributes written to the file to permanent
//! storage
inline int full_sync(int fd) {
  while (true) {
    ...
    if (BOOST_UNLIKELY(err < 0)) {
      err = errno;
      // POSIX says fsync can return EINTR
      // (https://pubs.opengroup.org/onlinepubs/9699919799/functions/fsync.html).
      // fcntl(F_FULLFSYNC) isn't documented to return EINTR, but it doesn't
      // hurt to check.
...
inline int data_sync(int fd) {
    ...
    !(defined(__APPLE__) && defined(__MACH__) && defined(F_FULLFSYNC))
  while (true) {
    ...
    if (BOOST_UNLIKELY(err != 0)) {
      ...
      // POSIX says fsync can return EINTR
      // (https://pubs.opengroup.org/onlinepubs/9699919799/functions/fsync.html).
      // It doesn't say so for fdatasync, but it is reasonable to expect it as
      // well.
      if (err == EINTR) continue;
...
// Min and max buffer sizes are selected to minimize the overhead from system
// calls. The values are picked based on coreutils cp(1) benchmarking data
// described here:
...
int copy_file_data_read_write_impl(int infile, int outfile, char* buf,
                                   std::size_t buf_size) {
#if defined(BOOST_FILESYSTEM_HAS_POSIX_FADVISE)
  ::posix_fadvise(infile, 0, 0, POSIX_FADV_SEQUENTIAL);
#endif

  // Don't use file size to limit the amount of data to copy since some
  // filesystems, like procfs or sysfs, provide files with generated content and
  // indicate that their size is zero or 4096. Just copy as much data as we can
  ...
  while (true) {
    ...
    for (ssize_t sz_wrote = 0; sz_wrote < sz_read;) {
      ssize_t sz = ::write(outfile, buf + sz_wrote,
...
//! copy_file implementation that uses read/write loop (fallback using a stack
//! buffer)
int copy_file_data_read_write_stack_buf(int infile, int outfile) {
  char stack_buf[min_read_write_buf_size];
  return copy_file_data_read_write_impl(infile, outfile, stack_buf,
                                        sizeof(stack_buf));
}
...
int copy_file_data_read_write(int infile, int outfile, uintmax_t size,
                              std::size_t blksize) {
  {
    ...
    if (buf_sz < blksize) buf_sz = blksize;
    if (buf_sz < min_read_write_buf_size) buf_sz = min_read_write_buf_size;
    if (buf_sz > max_read_write_buf_size) buf_sz = max_read_write_buf_size;
    const std::size_t buf_size = static_cast<std::size_t>(
        boost::core::bit_ceil(static_cast<uint_least32_t>(buf_sz)));
    boost::scoped_array<char> buf(new (std::nothrow) char[buf_size]);
    if (BOOST_LIKELY(!!buf.get()))
      return copy_file_data_read_write_impl(infile, outfile, buf.get(),
                                            buf_size);
  }
...
//! copy_file implementation that uses sendfile loop. Requires sendfile to
...
int copy_file_data_sendfile(int infile, int outfile, uintmax_t size,
  ...
  while (offset < size) {
    ...
    if (BOOST_UNLIKELY(sz < 0)) {
      ...
      if (offset == 0u) {
        // sendfile may fail with EINVAL if the underlying filesystem does not
        // support it
        if (err == EINVAL) {
        ...
        if (err == ENOSYS) {
          filesystem::detail::atomic_store_relaxed(copy_file_data,
...
//! copy_file implementation that uses copy_file_range loop. Requires
//! copy_file_range to support cross-filesystem copying.
int copy_file_data_copy_file_range(int infile, int outfile, uintmax_t size,
                                   std::size_t blksize) {
  // Although copy_file_range does not document any particular upper limit of
  // one transfer, still use some upper bound to guarantee that size_t is not
  // overflown in case if off_t is larger and the file size does not fit in
  // size_t.
  BOOST_CONSTEXPR_OR_CONST std::size_t max_batch_size = 0x7ffff000u;
  uintmax_t offset = 0u;
  while (offset < size) {
    ...
    if (size_left < static_cast<uintmax_t>(max_batch_size))
      size_to_copy = static_cast<std::size_t>(size_left);
    // Note: Use syscall directly to avoid depending on libc version.
    // copy_file_range is added in glibc 2.27. uClibc-ng does not have
    // copy_file_range as of the time of this writing (the latest uClibc-ng
    // release is 1.0.33).
    loff_t sz = ::syscall(__NR_copy_file_range, infile, (loff_t*)NULL, outfile,
                          (loff_t*)NULL, size_to_copy, (unsigned int)0u);
    if (BOOST_UNLIKELY(sz < 0)) {
      int err = errno;
      if (err == EINTR) continue;

      if (offset == 0u) {
        // copy_file_range may fail with EINVAL if the underlying filesystem
        // does not support it. In some RHEL/CentOS 7.7-7.8 kernel versions,
        // copy_file_range on NFSv4 is also known to return EOPNOTSUPP if the
        // remote server does not support COPY, despite that it is not a
        // documented error code. See
        // https://patchwork.kernel.org/project/linux-nfs/patch/20190411183418.4510-1-olga.kornievskaia@gmail.com/
...
#if defined(BOOST_FILESYSTEM_USE_SENDFILE) || \
    defined(BOOST_FILESYSTEM_USE_COPY_FILE_RANGE)

//! copy_file_data wrapper that tests if a read/write loop must be used for a
//! given filesystem
template <copy_file_data_t* CopyFileData>
int check_fs_type(int infile, int outfile, uintmax_t size,
                  std::size_t blksize) {
  {
    // Some filesystems have regular files with generated content. Such files
    // have arbitrary size, including zero, but have actual content. Linux
    // system calls sendfile or copy_file_range will not copy contents of such
    // files, so we must use a read/write loop to handle them.
    ...
    if (BOOST_UNLIKELY(
            sfs.f_type == PROC_SUPER_MAGIC || sfs.f_type == SYSFS_MAGIC ||
            sfs.f_type == TRACEFS_MAGIC || sfs.f_type == DEBUGFS_MAGIC)) {
    fallback_to_read_write:
      return copy_file_data_read_write(infile, outfile, size, blksize);
    }
  }
...
inline void init_copy_file_data_impl(unsigned int major_ver,
  ...
  if (major_ver > 2u ||
      (major_ver == 2u &&
       (minor_ver > 6u || (minor_ver == 6u && patch_ver >= 33u))))
    cfd = &check_fs_type<&copy_file_data_sendfile>;
#endif

#if defined(BOOST_FILESYSTEM_USE_COPY_FILE_RANGE)
  // Although copy_file_range appeared in Linux 4.5, it did not support
  // cross-filesystem copying until 5.3. copy_file_data_copy_file_range will
  // fallback to copy_file_data_sendfile if copy_file_range returns EXDEV.
...
struct syscall_initializer {
  syscall_initializer() {
    ...
    if (BOOST_UNLIKELY(::uname(&system_info) < 0)) return;

    unsigned int major_ver = 0u, minor_ver = 0u, patch_ver = 0u;
    int count = std::sscanf(system_info.release, "%u.%u.%u", &major_ver,
                            &minor_ver, &patch_ver);
    if (BOOST_UNLIKELY(count < 3)) return;
...
BOOST_ATTRIBUTE_UNUSED BOOST_FILESYSTEM_ATTRIBUTE_RETAIN const
    syscall_initializer syscall_init;

#endif  // defined(linux) || defined(__linux) || defined(__linux__)

//! remove() implementation
inline bool remove_impl(path const& p, fs::file_type type, error_code* ec) {
  if (type == fs::file_not_found) return false;
...
inline bool remove_impl(path const& p, error_code* ec) {
  // Since POSIX remove() is specified to work with either files or directories,
  // in a perfect world it could just be called. But some important real-world
  // operating systems (Windows, Mac OS, for example) don't implement the POSIX
  // spec. So we have to distinguish between files and directories and call
  // corresponding APIs to remove them.
  ...
  if (BOOST_UNLIKELY(type == fs::status_error)) {
    if (!ec)
      BOOST_FILESYSTEM_THROW(
          filesystem_error("boost::filesystem::remove", p, local_ec));
...
uintmax_t remove_all_impl(path const& p, error_code* ec) {
  fs::file_type type;
  {
    ...
    if (type == fs::file_not_found) return 0u;

    if (BOOST_UNLIKELY(type == fs::status_error)) {
      if (!ec)
        BOOST_FILESYSTEM_THROW(
            filesystem_error("boost::filesystem::remove_all", p, local_ec));
  ...
  if (type == fs::directory_file)  // but not a directory symlink
  {
    ...
    fs::detail::directory_iterator_construct(
        itr, p, static_cast<unsigned int>(directory_options::none), ec);
    if (ec && *ec) return static_cast<uintmax_t>(-1);
...
inline std::wstring wgetenv(const wchar_t* name) {
  // use a separate buffer since C++03 basic_string is not required to be
  // contiguous
...
inline HANDLE create_file_handle(path const& p, DWORD dwDesiredAccess,
                                 DWORD dwShareMode,
...
bool is_reparse_point_a_symlink(path const& p) {
  handle_wrapper h(create_file_handle(
      p, 0, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
      OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT,
      NULL));
  if (h.handle == INVALID_HANDLE_VALUE) return false;
  ...
  BOOL result = ::DeviceIoControl(h.handle, FSCTL_GET_REPARSE_POINT, NULL, 0,
                                  buf.get(), sizeof(*buf), &dwRetLen, NULL);
  ...
  return buf->rdb.ReparseTag == IO_REPARSE_TAG_SYMLINK
         ...
         // then the normal symlink mechanism will take care of recognizing them
         // as directories.
         //
...
inline std::size_t get_full_path_name(path const& src, std::size_t len,
...
inline fs::file_status process_status_failure(path const& p, error_code* ec) {
  ...
  if (!ec)
    BOOST_FILESYSTEM_THROW(filesystem_error(
        "boost::filesystem::status", p, error_code(errval, system_category())));
...
inline bool remove_impl(path const& p, DWORD attrs, error_code* ec) {
  // The following is similar to symlink_status(), except that it distinguishes
  // between symlinks to directories and to files, and also preserves the full
  // file attributes, which we'll need below.
  bool is_directory;
  if (BOOST_UNLIKELY(attrs == INVALID_FILE_ATTRIBUTES)) {
    ...
    is_directory =
        (attrs & FILE_ATTRIBUTE_DIRECTORY) && is_reparse_point_a_symlink(p);
  } else {
  ...
  if (is_directory) {
    ...
    if (is_read_only) {
      // DeleteFileW does not allow to remove a read-only file, so we have to
      // drop the attribute
...
uintmax_t remove_all_impl(path const& p, error_code* ec) {
  ...
  if (BOOST_UNLIKELY(attrs == INVALID_FILE_ATTRIBUTES)) {
    ...
    recurse = (attrs & FILE_ATTRIBUTE_DIRECTORY) != 0 &&
...
inline path convert_nt_path_to_win32_path(const wchar_t* nt_path,
                                          std::size_t size) {
  // https://googleprojectzero.blogspot.com/2016/02/the-definitive-guide-on-win32-to-nt.html
  // https://stackoverflow.com/questions/23041983/path-prefixes-and
  //
  // NT paths can be used to identify practically any named objects, devices,
  // files, local and remote shares, etc. The path starts with a leading
  // backslash and consists of one or more path elements separated with
  // backslashes. The set of characters allowed in NT path elements is
  // significantly larger than that of Win32 paths - basically, any character
  // except the backslash is allowed. Path elements are case-insensitive.
  //
  // NT paths that start with the "\??\" prefix are used to indicate the current
  // user's session namespace. The prefix indicates to the NT object manager to
  // lookup the object relative to "\Sessions\0\DosDevices\[Logon Authentication
  // ID]".
  //
  // There is also a special "\Global??\" prefix that refers to the system
  // logon. User's session directory shadows the system logon directory, so that
  // when the referenced object is not found in the user's namespace, system
  // logon is looked up instead.
  //
  // There is a symlink "Global" in the user's session namespace that refers to
  // the global namespace, so "\??\Global" effectively resolves to "\Global??".
  // This allows Win32 applications to directly refer to the system objects,
  // even if shadowed by the current user's logon object.
  //
  // NT paths can be used to reference not only local filesystems, but also
  // devices and remote shares identifiable via UNC paths. For this, there is a
  // special "UNC" device (which is a symlink to "\Device\Mup") in the system
  // logon namespace, so "\??\UNC\host\share" (or "\??\Global\UNC\host\share",
  // or "\Global??\UNC\host\share") is equivalent to "\\host\share".
  //
  // NT paths are not universally accepted by Win32 applications and APIs. For
  // example, Far supports paths starting with "\??\" and "\??\Global\" but not
  // with "\Global??\". As of Win10 21H1, File Explorer, cmd.exe and PowerShell
  // don't support any of these. Given this, and that NT paths have a different
  // set of allowed characters from Win32 paths, we should normally avoid
  // exposing NT paths to users that expect Win32 paths.
  //
  // In Boost.Filesystem we only deal with NT paths that come from reparse
  // points, such as symlinks and mount points, including directory junctions.
  // It was observed that reparse points created by junction.exe and mklink use
  // the "\??\" prefix for directory junctions and absolute symlink and
  // unqualified relative path for relative symlinks. Absolute paths are using
  // drive letters for mounted drives (e.g. "\??\C:\directory"), although it is
  // possible to create a junction to an directory using a different way of
  // identifying the filesystem (e.g.
  // "\??\Volume{00000000-0000-0000-0000-000000000000}\directory").
  // mklink does not support creating junctions pointing to a UNC path.
  // junction.exe does create a junction that uses a seemingly invalid syntax
  // like "\??\\\host\share", i.e. it basically does not expect an UNC path. It
  // is not known if reparse points that refer to a UNC path are considered
  // valid. There are reparse points created as mount points for local and
  // remote filsystems (for example, a cloud storage mounted in the local
  // filesystem). Such mount points have the form of
  // "\??\Volume{00000000-0000-0000-0000-000000000000}\",
  // "\??\Harddisk0Partition1\" or "\??\HarddiskVolume1\".
  // Reparse points that refer directly to a global namespace (through
  // "\??\Global\" or "\Global??\" prefixes) or devices (e.g.
  // "\Device\HarddiskVolume1") have not been observed so far.
  ...
  if (size >= 4u && nt_path[0] == path::preferred_separator &&
      nt_path[1] == questionmark && nt_path[2] == questionmark &&
      nt_path[3] == path::preferred_separator) {
    pos = 4u;

    // Check "Global"
    if ((size - pos) >= 6u && (nt_path[pos] == L'G' || nt_path[pos] == L'g') &&
  ...
  // Check for the "\Global??\" prefix
  else if (size >= 10u && nt_path[0] == path::preferred_separator &&
           ...
           nt_path[7] == questionmark && nt_path[8] == questionmark &&
  ...
  if (pos > 0u) {
    if ((size - pos) >= 2u &&
        (
            ...
            (detail::is_letter(nt_path[pos]) && nt_path[pos + 1u] == colon &&
             ((size - pos) == 2u ||
              detail::is_directory_separator(nt_path[pos + 2u]))) ||
            // Check for an "incorrect" syntax for UNC path junction points
            (detail::is_directory_separator(nt_path[pos]) &&
             detail::is_directory_separator(nt_path[pos + 1u]) &&
             ((size - pos) == 2u ||
              !detail::is_directory_separator(nt_path[pos + 2u]))))) {
    ...
    static const wchar_t win32_path_prefix[4u] = {
        path::preferred_separator, path::preferred_separator, questionmark,
        path::preferred_separator};

    // Check for a UNC path
    if ((size - pos) >= 4u && (nt_path[pos] == L'U' || nt_path[pos] == L'u') &&
    ...
    // This is some other NT path, possibly a volume mount point. Replace the NT
    // prefix with a Win32 filesystem prefix "\\?\".
    win32_path.assign(win32_path_prefix, win32_path_prefix + 4);
    if (global_namespace) {
      static const wchar_t win32_path_global_prefix[7u] = {
          L'G', L'l', L'o', L'b', L'a', L'l', path::preferred_separator};
...
BOOST_FILESYSTEM_INIT_FUNC init_winapi_func_ptrs() {
  ...
  if (BOOST_LIKELY(!!h)) {
    create_hard_link_api = (PtrCreateHardLinkW)boost::winapi::get_proc_address(
        h, "CreateHardLinkW");
    create_symbolic_link_api =
        (PtrCreateSymbolicLinkW)boost::winapi::get_proc_address(
            h, "CreateSymbolicLinkW");
  }
...
__declspec(allocate(".CRT$XCL"))
    BOOST_ATTRIBUTE_UNUSED BOOST_FILESYSTEM_ATTRIBUTE_RETAIN extern const
    init_func_ptr_t p_init_winapi_func_ptrs = &init_winapi_func_ptrs;
...
BOOST_ATTRIBUTE_UNUSED
    BOOST_FILESYSTEM_ATTRIBUTE_RETAIN extern const init_func_ptr_t
        p_init_winapi_func_ptrs = &init_winapi_func_ptrs;
#pragma data_seg()
#if (_MSC_VER >= 1300)  // 1300 == VC++ 7.0
...
//! Makes sure the global initializer pointers are referenced and not removed by
//! linker
...
BOOST_FILESYSTEM_INIT_PRIORITY(BOOST_FILESYSTEM_FUNC_PTR_INIT_PRIORITY)
BOOST_ATTRIBUTE_UNUSED BOOST_FILESYSTEM_ATTRIBUTE_RETAIN const
    winapi_func_ptrs_initializer winapi_func_ptrs_init
#if !defined(BOOST_NO_CXX11_UNIFIED_INITIALIZATION_SYNTAX)
    {};
#else   // !defined(BOOST_NO_CXX11_UNIFIED_INITIALIZATION_SYNTAX)
...
path absolute(path const& p, path const& base, system::error_code* ec) {
  ...
  if (!p_relative_path.empty()) res /= p_relative_path;
...
path canonical(path const& p, path const& base, system::error_code* ec) {
  ...
  if (st.type() == fs::file_not_found) {
    local_ec =
        system::errc::make_error_code(system::errc::no_such_file_or_directory);
    goto fail_local_ec;
  } else if (local_ec) {
  fail_local_ec:
    if (!ec)
      BOOST_FILESYSTEM_THROW(
          filesystem_error("boost::filesystem::canonical", source, local_ec));
  ...
  path result;
  while (true) {
    for (path::iterator itr(source.begin()), end(source.end()); itr != end;
         ++itr) {
      if (*itr == dot_p) continue;
      if (*itr == dot_dot_p) {
        if (result != root) result.remove_filename();
        continue;
      }

      if (itr->size() == 1u &&
          detail::is_directory_separator(itr->native()[0])) {
        // Convert generic separator returned by the iterator for the root
        // directory to the preferred separator. This is important on Windows,
        // as in some cases, like paths for network shares and cloud storage
        // mount points GetFileAttributesW will return "file not found" if the
      ...
      if (!result.is_absolute()) continue;

      st = detail::symlink_status(result, ec);
      if (ec && *ec) goto return_empty_path;

      if (is_symlink(st)) {
        if (symlinks_allowed == 0) {
          local_ec = system::errc::make_error_code(
              system::errc::too_many_symbolic_link_levels);
          goto fail_local_ec;
        }
        ...
        } else  // link is relative
        {
          ...
          if (link == dot_p) continue;

          path new_source(result);
          new_source /= link;
          for (++itr; itr != end; ++itr) {
            if (*itr != dot_p) new_source /= *itr;
          }
          source = new_source;
        }
...
BOOST_FILESYSTEM_DECL
void copy(path const& from, path const& to, unsigned int options,
          system::error_code* ec) {
  BOOST_ASSERT(
      (((options &
         static_cast<unsigned int>(copy_options::overwrite_existing)) != 0u) +
       ((options & static_cast<unsigned int>(copy_options::skip_existing)) !=
        0u) +
       ((options & static_cast<unsigned int>(copy_options::update_existing)) !=
        0u)) <= 1);

  BOOST_ASSERT(
      (((options & static_cast<unsigned int>(copy_options::copy_symlinks)) !=
        0u) +
       ((options & static_cast<unsigned int>(copy_options::skip_symlinks)) !=
        0u)) <= 1);

  BOOST_ASSERT(
      (((options & static_cast<unsigned int>(copy_options::directories_only)) !=
        0u) +
       ((options & static_cast<unsigned int>(copy_options::create_symlinks)) !=
        0u) +
       ((options & static_cast<unsigned int>(
                       copy_options::create_hard_links)) != 0u)) <= 1);
  ...
  if ((options & (static_cast<unsigned int>(copy_options::copy_symlinks) |
                  ...
                  static_cast<unsigned int>(copy_options::create_symlinks))) !=
      0u) {
  ...
  if (!exists(from_stat)) {
    emit_error(BOOST_ERROR_FILE_NOT_FOUND, from, to, ec,
  ...
  if (is_symlink(from_stat)) {
    ...
    if ((options & static_cast<unsigned int>(copy_options::directories_only)) !=
        0u)
      return;

    if ((options & static_cast<unsigned int>(copy_options::create_symlinks)) !=
        0u) {
      const path* pfrom = &from;
      path relative_from;
      if (!from.is_absolute()) {
        // Try to generate a relative path from the target location to the
        // original file
    ...
    if ((options &
         static_cast<unsigned int>(copy_options::create_hard_links)) != 0u) {
    ...
    // Note: local_ec may be set by (symlink_)status() even in some non-fatal
    // situations, e.g. when the file does not exist.
    //       OTOH, when it returns status_error, then a real error have happened
    //       and it must have set local_ec.
    if (to_stat.type() == fs::status_error) {
      if (!ec)
        BOOST_FILESYSTEM_THROW(
            filesystem_error("boost::filesystem::copy", from, to, local_ec));
      *ec = local_ec;
      return;
    }
    ...
    // Note: ec may be set by (symlink_)status() even in some non-fatal
    // situations, e.g. when the file does not exist.
    //       OTOH, when it returns status_error, then a real error have happened
    //       and it must have set local_ec.
    if (to_stat.type() == fs::status_error) {
    ...
    if ((options & static_cast<unsigned int>(copy_options::recursive)) != 0u ||
        options == 0u) {
      fs::directory_iterator itr;
      detail::directory_iterator_construct(
          itr, from, static_cast<unsigned int>(directory_options::none), ec);
      if (ec && *ec) return;

      const fs::directory_iterator end_dit;
      while (itr != end_dit) {
        path const& p = itr->path();
        // Set _detail_recursing flag so that we don't recurse more than for one
        // level deeper into the directory if options are copy_options::none
        detail::copy(p, to / p.filename(),
                     options | static_cast<unsigned int>(
                                   copy_options::_detail_recursing),
                     ec);
        if (ec && *ec) return;
  ...
  fail:
    emit_error(BOOST_ERROR_NOT_SUPPORTED, from, to, ec,
               "boost::filesystem::copy");
  }
}

BOOST_FILESYSTEM_DECL
bool copy_file(path const& from, path const& to, unsigned int options,
               error_code* ec) {
  BOOST_ASSERT(
      (((options &
         static_cast<unsigned int>(copy_options::overwrite_existing)) != 0u) +
  ...
  // Note: Declare fd_wrappers here so that errno is not clobbered by close()
  // that may be called in fd_wrapper destructors
...
#if defined(BOOST_FILESYSTEM_USE_STATX)
  unsigned int statx_data_mask =
      STATX_TYPE | STATX_MODE | STATX_INO | STATX_SIZE;
  if ((options & static_cast<unsigned int>(copy_options::update_existing)) !=
      0u)
    statx_data_mask |= STATX_MTIME;

  struct ::statx from_stat;
  if (BOOST_UNLIKELY(invoke_statx(infile.fd, "",
                                  AT_EMPTY_PATH | AT_NO_AUTOMOUNT,
                                  statx_data_mask, &from_stat) < 0)) {
  fail_errno:
    err = errno;
    goto fail;
  }

  if (BOOST_UNLIKELY((from_stat.stx_mask & statx_data_mask) !=
                     statx_data_mask)) {
    err = ENOSYS;
    goto fail;
  }
...
#if !defined(__wasm)
  // Enable writing for the newly created files. Having write permission set is
  // important e.g. for NFS, which checks the file permission on the server,
  // even if the client's file descriptor supports writing.
  to_mode |= S_IWUSR;
#endif
  ...
  if ((options & static_cast<unsigned int>(copy_options::update_existing)) !=
      0u) {
    // Try opening the existing file without truncation to test the modification
    // time later
    while (true) {
      ...
      if (outfile.fd < 0) {
        ...
        if (err == ENOENT) goto create_outfile;
  ...
  create_outfile:
    ...
    while (true) {
      ...
      if (outfile.fd < 0) {
        ...
        if (err == EEXIST && (options & static_cast<unsigned int>(
                                            copy_options::skip_existing)) != 0u)
          return false;
  ...
  if ((oflag & O_TRUNC) == 0) {
    // O_TRUNC is not set if copy_options::update_existing is set and an
    // existing file was opened.
    statx_data_mask |= STATX_MTIME;
  }

  struct ::statx to_stat;
  if (BOOST_UNLIKELY(invoke_statx(outfile.fd, "",
                                  AT_EMPTY_PATH | AT_NO_AUTOMOUNT,
                                  statx_data_mask, &to_stat) < 0))
    goto fail_errno;
...
#else
  struct ::stat to_stat;
  if (BOOST_UNLIKELY(::fstat(outfile.fd, &to_stat) != 0)) goto fail_errno;
#endif
  ...
  if ((oflag & O_TRUNC) == 0) {
    // O_TRUNC is not set if copy_options::update_existing is set and an
    // existing file was opened. We need to check the last write times.
#if defined(BOOST_FILESYSTEM_USE_STATX)
    if (from_stat.stx_mtime.tv_sec < to_stat.stx_mtime.tv_sec ||
        (from_stat.stx_mtime.tv_sec == to_stat.stx_mtime.tv_sec &&
         from_stat.stx_mtime.tv_nsec <= to_stat.stx_mtime.tv_nsec))
      return false;
    ...
    if (from_stat.st_mtime < to_stat.st_mtime ||
        (from_stat.st_mtime == to_stat.st_mtime &&
         from_stat.BOOST_FILESYSTEM_STAT_ST_MTIMENSEC <=
             to_stat.BOOST_FILESYSTEM_STAT_ST_MTIMENSEC))
      return false;
#else
    if (from_stat.st_mtime <= to_stat.st_mtime) return false;
#endif

    if (BOOST_UNLIKELY(::ftruncate(outfile.fd, 0) != 0)) goto fail_errno;
  }

  // Note: Use block size of the target file since it is most important for
  // writing performance.
  err = filesystem::detail::atomic_load_relaxed(
      filesystem::detail::copy_file_data)(
      infile.fd, outfile.fd, get_size(from_stat), get_blksize(to_stat));
  if (BOOST_UNLIKELY(err != 0))
  ...
  if (to_mode != from_mode) {
    if (BOOST_UNLIKELY(::fchmod(outfile.fd, from_mode) != 0)) goto fail_errno;
  }
#endif

  if ((options & (static_cast<unsigned int>(copy_options::synchronize_data) |
                  static_cast<unsigned int>(copy_options::synchronize))) !=
      0u) {
    ...
    if (BOOST_UNLIKELY(err != 0)) goto fail;
  }

  // We have to explicitly close the output file descriptor in order to handle a
  // possible error returned from it. The error may indicate a failure of a
  // prior write operation.
  err = close_fd(outfile.fd);
  outfile.fd = -1;
  if (BOOST_UNLIKELY(err < 0)) {
    err = errno;
    // EINPROGRESS is an allowed error code in future POSIX revisions, according
    // to https://www.austingroupbugs.net/view.php?id=529#c1200.
    if (err != EINTR && err != EINPROGRESS) goto fail;
  }
  ...
  DWORD copy_flags = 0u;
  if ((options & static_cast<unsigned int>(copy_options::overwrite_existing)) ==
          0u ||
      (options & static_cast<unsigned int>(copy_options::skip_existing)) !=
          0u) {
  ...
  if ((options & static_cast<unsigned int>(copy_options::update_existing)) !=
      0u) {
    // Create handle_wrappers here so that CloseHandle calls don't clobber error
    // code returned by GetLastError
    handle_wrapper hw_from, hw_to;

    hw_from.handle = create_file_handle(
        from.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    if (!::GetFileTime(hw_from.handle, 0, 0, &lwt_from)) goto fail_last_error;

    hw_to.handle = create_file_handle(
        to.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
        0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0);

    if (hw_to.handle != INVALID_HANDLE_VALUE) {
      FILETIME lwt_to;
      if (!::GetFileTime(hw_to.handle, 0, 0, &lwt_to)) goto fail_last_error;

      ULONGLONG tfrom =
          (static_cast<ULONGLONG>(lwt_from.dwHighDateTime) << 32) |
          static_cast<ULONGLONG>(lwt_from.dwLowDateTime);
      ULONGLONG tto = (static_cast<ULONGLONG>(lwt_to.dwHighDateTime) << 32) |
                      static_cast<ULONGLONG>(lwt_to.dwLowDateTime);
  ...
  struct local {
    ...
    static DWORD WINAPI on_copy_file_progress(
        ...
        DWORD stream_number, DWORD callback_reason, HANDLE from_handle,
        HANDLE to_handle, LPVOID ctx) {
      // For each stream, CopyFileExW will open a separate pair of file handles,
      // so we need to flush each stream separately.
  ...
  BOOL res = ::CopyFileExW(from.c_str(), to.c_str(), cb, cb_ctx, &cancelled,
                           copy_flags);
  DWORD err;
  if (BOOST_UNLIKELY(!res)) {
    err = ::GetLastError();
...
bool create_directories(path const& p, system::error_code* ec) {
  ...
  for (path fname = parent.filename(); parent.has_relative_path();
    ...
    if (!fname.empty() && fname != dot_p && fname != dot_dot_p) {
      ...
      if (existing_status.type() == directory_file) {
        ...
        if (!ec)
          BOOST_FILESYSTEM_THROW(filesystem_error(
              "boost::filesystem::create_directories", p, parent, local_ec));
        *ec = local_ec;
        return false;
      }
    }
...
bool create_directory(path const& p, const path* existing, error_code* ec) {
  ...
  if (existing) {
    ...
    if (BOOST_UNLIKELY(invoke_statx(AT_FDCWD, existing->c_str(),
                                    AT_NO_AUTOMOUNT, STATX_TYPE | STATX_MODE,
                                    &existing_stat) < 0)) {
      emit_error(errno, p, *existing, ec,
                 "boost::filesystem::create_directory");
      return false;
    }

    if (BOOST_UNLIKELY((existing_stat.stx_mask & (STATX_TYPE | STATX_MODE)) !=
                       (STATX_TYPE | STATX_MODE))) {
      emit_error(BOOST_ERROR_NOT_SUPPORTED, p, *existing, ec,
                 "boost::filesystem::create_directory");
      return false;
    }
#else
    ...
    if (!S_ISDIR(existing_mode)) {
      emit_error(ENOTDIR, p, *existing, ec,
                 "boost::filesystem::create_directory");
      return false;
    }
  ...
  if (is_directory(p, dummy)) return false;
...
void copy_directory(path const& from, path const& to, system::error_code* ec) {
  ...
  int err;
  struct ::statx from_stat;
  if (BOOST_UNLIKELY(invoke_statx(AT_FDCWD, from.c_str(), AT_NO_AUTOMOUNT,
                                  STATX_TYPE | STATX_MODE, &from_stat) < 0)) {
  fail_errno:
    err = errno;
  fail:
  ...
  if (BOOST_UNLIKELY((from_stat.stx_mask & (STATX_TYPE | STATX_MODE)) !=
                     (STATX_TYPE | STATX_MODE))) {
...
BOOST_FILESYSTEM_DECL
void create_directory_symlink(path const& to, path const& from,
                              system::error_code* ec) {
  if (ec) ec->clear();
  ...
  if (BOOST_UNLIKELY(err < 0)) {
    err = errno;
    emit_error(err, to, from, ec,
               "boost::filesystem::create_directory_symlink");
  }
#else
  // see if actually supported by Windows runtime dll
  if (!create_symbolic_link_api) {
    emit_error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec,
               "boost::filesystem::create_directory_symlink");
    return;
  }
  ...
  if (!create_symbolic_link_api(
    ...
    emit_error(BOOST_ERRNO, to, from, ec,
               "boost::filesystem::create_directory_symlink");
  }
#endif
}
...
void create_hard_link(path const& to, path const& from, error_code* ec) {
  ...
  error(!BOOST_CREATE_HARD_LINK(from.c_str(), to.c_str()) ? BOOST_ERRNO : 0, to,
        from, ec, "boost::filesystem::create_hard_link");
}
...
void create_symlink(path const& to, path const& from, error_code* ec) {
  ...
  if (!create_symbolic_link_api(from.c_str(), to.c_str(),
                                SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE)) {
...
path current_path(error_code* ec) {
  ...
  // Windows CE has no current directory, so everything's relative to the root
  // of the directory tree. WASI also does not support current path.
  ...
  struct local {
    ...
    (defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__))
                    // bug in some versions of the Metrowerks C lib on the Mac:
                    // wrong errno set
                    && err != 0
#endif
                    )
  ...
  if (BOOST_LIKELY(!!p)) {
    ...
    for (std::size_t path_max = sizeof(small_buf) * 2u;;
         path_max *= 2u)  // loop 'til buffer large enough
    {
...
#if defined(UNDER_CE) || defined(__wasm)
  emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec,
             "boost::filesystem::current_path");
#else
...
bool equivalent(path const& p1, path const& p2, system::error_code* ec) {
  ...
  if (BOOST_LIKELY(e2 == 0)) {
    ...
    fail_unsupported:
      emit_error(BOOST_ERROR_NOT_SUPPORTED, p1, p2, ec,
  ...
  handle_wrapper h2(create_file_handle(
      p2.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
      OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0));

  handle_wrapper h1(create_file_handle(
      p1.c_str(), 0, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
      OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0));
...
uintmax_t file_size(path const& p, error_code* ec) {
  ...
  if (BOOST_UNLIKELY(invoke_statx(AT_FDCWD, p.c_str(), AT_NO_AUTOMOUNT,
                                  STATX_TYPE | STATX_SIZE, &path_stat) < 0)) {
    emit_error(errno, p, ec, "boost::filesystem::file_size");
    return static_cast<uintmax_t>(-1);
  }

  if (BOOST_UNLIKELY((path_stat.stx_mask & (STATX_TYPE | STATX_SIZE)) !=
                         (STATX_TYPE | STATX_SIZE) ||
                     !S_ISREG(path_stat.stx_mode))) {
    emit_error(BOOST_ERROR_NOT_SUPPORTED, p, ec,
...
bool is_empty(path const& p, system::error_code* ec) {
  ...
  if (S_ISDIR(get_mode(path_stat))) return is_empty_directory(p, ec);
  ...
  return S_ISDIR(get_mode(path_stat)) ? is_empty_directory(p, ec)
...
std::time_t creation_time(path const& p, system::error_code* ec) {
...
#elif defined(BOOST_FILESYSTEM_STAT_ST_BIRTHTIME) && \
    defined(BOOST_FILESYSTEM_STAT_ST_BIRTHTIMENSEC)
  struct ::stat st;
  ...
  if (BOOST_UNLIKELY(!::GetFileTime(hw.handle, &ct, NULL, NULL))) goto fail;
...
std::time_t last_write_time(path const& p, system::error_code* ec) {
  ...
  if (BOOST_UNLIKELY(!::GetFileTime(hw.handle, NULL, NULL, &lwt))) goto fail;
...
void last_write_time(path const& p, const std::time_t new_time,
  ...
  if (BOOST_UNLIKELY(!::SetFileTime(hw.handle, 0, 0, &lwt))) goto fail;
...
void permissions(path const& p, perms prms, system::error_code* ec) {
  ...
  if (local_ec) {
    if (!ec)
      BOOST_FILESYSTEM_THROW(
          filesystem_error("boost::filesystem::permissions", p, local_ec));
  ...
  if (prms & add_perms)
    ...
    // Solaris (SunPro and gcc) only support fchmodat() on Solaris 11 and
    // higher, and a runtime check is too much trouble. Linux does not support
    // permissions on symbolic links and has no plans to support them in the
    // future.  The chmod() code is thus more practical, rather than always
    // hitting ENOTSUP when sending in AT_SYMLINK_NO_FOLLOW.
...
#endif
  {
    ...
    if (!ec) {
      BOOST_FILESYSTEM_THROW(
          filesystem_error("boost::filesystem::permissions", p,
...
path read_symlink(path const& p, system::error_code* ec) {
  ...
  } else if (BOOST_LIKELY(static_cast<std::size_t>(result) <
                          sizeof(small_buf))) {
  ...
  if (!error(::DeviceIoControl(h.handle, FSCTL_GET_REPARSE_POINT, 0, 0,
    ...
    switch (buf->rdb.ReparseTag) {
      ...
      case IO_REPARSE_TAG_SYMLINK:
        ...
        // Note: iff info.rdb.SymbolicLinkReparseBuffer.Flags &
    ...
    symlink_path = convert_nt_path_to_win32_path(
        buffer + offset / sizeof(wchar_t), len / sizeof(wchar_t));
  }
#endif
...
path relative(path const& p, path const& base, error_code* ec) {
  ...
  if (!p.is_absolute() || !base.is_absolute()) {
    ...
    fail_local_ec:
      if (!ec)
        BOOST_FILESYSTEM_THROW(
            filesystem_error("boost::filesystem::relative", p, base, local_ec));
  ...
  if (BOOST_UNLIKELY(!!local_ec)) goto fail_local_ec;
...
void rename(path const& old_p, path const& new_p, error_code* ec) {
  error(!BOOST_MOVE_FILE(old_p.c_str(), new_p.c_str()) ? BOOST_ERRNO : 0, old_p,
        new_p, ec, "boost::filesystem::rename");
}
...
void resize_file(path const& p, uintmax_t size, system::error_code* ec) {
  ...
  if (BOOST_UNLIKELY(
    ...
    emit_error(system::errc::file_too_large, p, ec,
...
space_info space(path const& p, error_code* ec) {
  space_info info;
  // Initialize members to -1, as required by C++20 [fs.op.space]/1 in case of
  // error
  ...
  // GetDiskFreeSpaceExW requires a directory path, which is unlike statvfs,
  // which accepts any file. To work around this, test if the path refers to a
  // directory and use the parent directory if not.
  error_code local_ec;
  file_status status = detail::status(p, &local_ec);
  if (status.type() == fs::status_error ||
      status.type() == fs::file_not_found) {
  fail_local_ec:
    if (!ec)
      BOOST_FILESYSTEM_THROW(
          filesystem_error("boost::filesystem::space", p, local_ec));
    *ec = local_ec;
    return info;
  }
  ...
  if (!is_directory(status)) {
    ...
    if (ec && *ec) return info;

    status = detail::symlink_status(p, &local_ec);
    if (status.type() == fs::status_error) goto fail_local_ec;
    if (is_symlink(status)) {
      // We need to resolve the symlink so that we report the space for the
      // symlink target
    ...
    if (dir_path.empty()) {
      // The original path was just a filename, which is a relative path wrt.
      // current directory
      dir_path = cur_path;
    }
  }
  ...
  if (str.size() >= 2u && detail::is_directory_separator(str[0]) &&
      detail::is_directory_separator(str[1]) &&
      !detail::is_directory_separator(*(str.end() - 1)))
    str.push_back(path::preferred_separator);

  ULARGE_INTEGER avail, total, free;
  if (!error(::GetDiskFreeSpaceExW(str.c_str(), &avail, &total, &free) == 0, p,
...
file_status status(path const& p, error_code* ec) {
...
#if defined(BOOST_FILESYSTEM_USE_STATX)
  struct ::statx path_stat;
  int err = invoke_statx(AT_FDCWD, p.c_str(), AT_NO_AUTOMOUNT,
                         STATX_TYPE | STATX_MODE, &path_stat);
#else
  struct ::stat path_stat;
  ...
  if (err != 0) {
    ...
    if (!ec)
      BOOST_FILESYSTEM_THROW(filesystem_error(
          "boost::filesystem::status", p, error_code(err, system_category())));

    return fs::file_status(fs::status_error);
  }

#if defined(BOOST_FILESYSTEM_USE_STATX)
  if (BOOST_UNLIKELY((path_stat.stx_mask & (STATX_TYPE | STATX_MODE)) !=
                     (STATX_TYPE | STATX_MODE))) {
...
file_status symlink_status(path const& p, error_code* ec) {
  ...
  struct ::statx path_stat;
  int err =
      invoke_statx(AT_FDCWD, p.c_str(), AT_SYMLINK_NOFOLLOW | AT_NO_AUTOMOUNT,
                   STATX_TYPE | STATX_MODE, &path_stat);
#else
  struct ::stat path_stat;
  ...
  if (err != 0) {
    ...
    if (!ec)
      BOOST_FILESYSTEM_THROW(
          filesystem_error("boost::filesystem::symlink_status", p,
                           error_code(err, system_category())));
...
path temp_directory_path(system::error_code* ec) {
  ...
  if (BOOST_UNLIKELY(ec && *ec)) return path();
  if (BOOST_UNLIKELY(!is_directory(status))) goto fail_not_dir;
  ...
  if (p.empty()) {
    // use a separate buffer since in C++03 a string is not required to be
    // contiguous
  ...
  boost::scoped_array<wchar_t> buf(new wchar_t[size]);
  if (::GetTempPathW(size, buf.get()) == 0) goto fail;
  ...
  if (!is_directory(status)) {
    error(ERROR_PATH_NOT_FOUND, p, ec,
...
path weakly_canonical(path const& p, path const& base, system::error_code* ec) {
  ...
  for (; !head.empty(); --itr) {
    ...
    if (BOOST_UNLIKELY(head_status.type() == fs::status_error)) {
      if (!ec)
        BOOST_FILESYSTEM_THROW(filesystem_error(
            "boost::filesystem::weakly_canonical", head, local_ec));

      *ec = local_ec;
      return path();
    }

    if (head_status.type() != fs::file_not_found) break;
  ...
  // On Windows, filesystem APIs such as GetFileAttributesW perform lexical path
  // normalization internally. As a result, a path like "c:\a\.." can be
  // reported as present even if "c:\a" is not. This would break canonical, as
  // symlink_status that it calls internally would report an error that the file
  // at the intermediate path does not exist. To avoid this, scan the initial
  // path in the forward direction. Also, operate on paths with preferred
  // separators. This can be important on Windows since GetFileAttributesW,
  // which is called in status() may return "file not found" for paths to
  // network shares and mounted cloud storages that have forward slashes as
  // separators.
  ...
  for (; itr != p_end; ++itr) {
    ...
    if (p_elem.size() == 1u &&
        detail::is_directory_separator(p_elem.native()[0])) {
      // Convert generic separator returned by the iterator for the root
      // directory to the preferred separator.
...
#if defined(BOOST_WINDOWS_API)
    if (tail_elem.size() == 1u &&
        detail::is_directory_separator(tail_elem.native()[0])) {
      // Convert generic separator returned by the iterator for the root
      // directory to the preferred separator.
  ...
  if (head.empty()) return p.lexically_normal();
  ...
  if (BOOST_LIKELY(!tail.empty())) {
    ...
    if (tail_has_dots) return head.lexically_normal();
  }
